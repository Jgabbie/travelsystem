...
const createService = async (req, res) => {
    ...
    try {
        ...
        logAction('Service created', { userId, serviceId: newService._id });
        res.status(201).json(newService);
    } catch (error) {
        logAction('Error creating service', { userId, error: error.message });
...
const getAllServices = async (req, res) => {
    ...
    try {
        ...
        logAction('Error retrieving services', { userId, error: error.message });
...
const updateService = async (req, res) => {
    ...
    try {
        ...
        if (!updatedService) {
            logAction('Service not found for update', { userId, serviceId: id });
            return res.status(404).json({ message: 'Service not found' });
        }
        logAction('Service updated', { userId, serviceId: id });
        res.status(200).json(updatedService);
    } catch (error) {
        logAction('Error updating service', { userId, serviceId: id, error: error.message });
...
const deleteService = async (req, res) => {
    ...
    try {
        ...
        if (!deletedService) {
            logAction('Service not found for deletion', { userId, serviceId: id });
            return res.status(404).json({ message: 'Service not found' });
        }
        logAction('Service deleted', { userId, serviceId: id });
        res.status(200).json({ message: 'Service deleted successfully' });
    } catch (error) {
        logAction('Error deleting service', { userId, serviceId: id, error: error.message });
