...
export default function VisaApplication() {
    ...
    useEffect(() => {
        ...
        const fetchApplication = async () => {
            ...
            try {
                ...
                notificationApi.error({ title: 'Failed to load visa application details', placement: 'topRight' });
            } finally {
                setLoading(false);
            }
        };
    ...
    const handleSubmitDocuments = async () => {
        if (uploading) {
            notificationApi.warning({
                title: "Please wait until uploads finish",
                placement: "topRight",
            });
            return;
        }
    ...
    const handleSubmitPayment = async () => {
        ...
        if (isDeliveryFeeStage && deliveryFeeAmount <= 0) {
            notificationApi.warning({ title: 'Delivery fee is not available yet. Please wait for admin to send it.', placement: 'topRight' });
            return;
        }
    ...
    const handleUpload = (requirementKey) => async ({ file, onSuccess, onError }) => {
        ...
        try {
            ...
            notificationApi.success({ title: 'File ready for submission', placement: 'topRight' });
            onSuccess('ok');
        } catch (err) {
            console.error(err);
            notificationApi.error({ title: 'Failed to process file', placement: 'topRight' });
            onError(err);
        } finally {
            setUploading(false);
        }
    ...
    const handleReleaseOption = async () => {
        if (!releaseOption) {
            notificationApi.warning({ title: 'Please select a release option first.', placement: 'topRight' });
            return
        }

        if (releaseOption === 'delivery' && deliveryAddress.trim() === "") {
            notificationApi.warning({ title: 'Please provide a delivery address in your profile settings before choosing delivery option.', placement: 'topRight' });
            return;
        }
        ...
        try {
            ...
            notificationApi.error({ title: 'Failed to update release option.', placement: 'topRight' });
        }
    }
                    ...
                    <div className='visaapplication-container'>
                        ...
                        {application && (
                            <>
                                ...
                                {/* PAYMENT COMPLETED */}
                                {statusValue && statusValue.toLowerCase() === 'payment completed' && application?.onPenalty === false && (
                                    <div style={{ marginBottom: 24, borderLeft: '4px solid #00bf63', backgroundColor: '#f6ffed', padding: 16, paddingBottom: 40, paddingTop: 40, borderRadius: 8, }} >
                                ...
                                {application?.onPenalty && application.secondChance === false &&
                                    (statusValue?.trim().toLowerCase() === 'payment completed' ||
                                        statusValue?.trim().toLowerCase() === 'documents uploaded') && (
                                        <div style={{ marginBottom: 24, borderLeft: '4px solid #ff4d4f', backgroundColor: '#fff1f0', padding: 16, paddingBottom: 40, paddingTop: 40, borderRadius: 8, }}>
                                            <h2 style={{ marginBottom: 10, fontSize: 20, fontWeight: 600, color: '#ff4d4f', }}>
