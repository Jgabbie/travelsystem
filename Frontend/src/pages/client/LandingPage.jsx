...
export default function LandingPage() {
            ...
            try {
                ...
                const packages = (response || []).map((pkg) => ({
                    ...
                    discountPercent: Number.isFinite(Number(pkg.packageDiscountPercent))
                        ...
                        : Number.isFinite(Number(pkg.discountPercent))
                            ? Number(pkg.discountPercent)
                            : Number.isFinite(Number(pkg.discountPercentOff))
                                ? Number(pkg.discountPercentOff)
                                : 0
                }))
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setRecentTourIndex(prev => (prev + 1) % recentTours.length);
        }, 4000);
                    ...
                    return {
                        ...
                        discountPercent: Number.isFinite(
                            ...
                            : Number.isFinite(Number(packageData?.discountPercent))
                                ...
                                : Number.isFinite(Number(pkg?.packageDiscountPercent))
                                    ? Number(pkg.packageDiscountPercent)
                                    : Number.isFinite(Number(pkg?.discountPercent))
                                        ? Number(pkg.discountPercent)
                                        : 0
    ...
    const recentTourSlides = [];

    for (
        let i = 0;
        i < recentTours.length;
        i += popularCardsPerView
    ) {
        recentTourSlides.push(
            recentTours.slice(i, i + popularCardsPerView)
        );
    }
    ...
    useEffect(() => {
        const fetchRecentTours = async () => {
            ...
            try {
                const response = await apiFetch.get("/recent-tours/get-recent-tours");

                setRecentTours(response || []);
            } catch (err) {
                console.error("Failed to fetch recent tours:", err);
                setRecentTours([]);
            } finally {
                ...
                <div ref={packagesRef}>
                    ...
                    <div className='for-you-section'>
                        ...
                        {!auth && !authLoading ? (
                            ...
                            <div className="for-you-empty-state">
                                <p className="for-you-status-text">
                                    {forYouError}
                                </p>

                                <Button
                                    type="primary"
                                    className="for-you-login-button"
